...
(function (window) {
    ...
    wallet.prototype = {
        ...
        getAllUnspentInputs: function (address) {
            ...
            return util.getJSON(host + address + '/full?unspentOnly=true&limit=50').then(function (json) {
                var unspent_inputs = [];
    ...
    wallet.prototype.mulitpleOutputsSend = function (txOutputs, fee, password) {
        return new Promise(function (resolve, reject) {
            ...
            if (decryptedPrivateKey) {
                ...
                ret.getAllUnspentInputs(address).then(function (inputs) {
                //util.getJSON('https://blockchain.info/unspent?address=' + address).then(function (json) {
                    ...
                    // for (i = 0; i < inputs.length; i++) {
                    //     tx.addInput(inputs[i].tx_hash, inputs[i].output_index);
                    // }

                    tx.addInput('2b0ce096081139ddceece2934244f9bb902d7f7d6750251b62ddcbba74681112', 1);
                    //tx.addInput('0000000000000000080b6d5d0cce43f939c88c472da278b3f99ef489a1de7137', 1);
                    //tx.addInput('5d095ca54aba01e1712928f342724c63f0e9fd08c1e82d927f9890d593bc8e8b', 1);
                    //tx.addInput('12116874bacbdd621b2550677d7f2d90bbf9444293e2eccedd39110896e00c2b', 1);
                    tx.addOutput('1FhCLQK2ZXtCUQDtG98p6fVH7S6mxAsEey', 85299);
                    tx.sign(0, keyPair)
                    ...
                    //
                    //       // debugger;
                    //       //sendTx.addInput(inputs[i].tx_hash, inputs[i].output_index);
                    ...
                    //  var data = Crypto.util.bytesToHex(sendTx.serialize());

                    console.log(data);
                    debugger;
                    util.post('https://api.blockcypher.com/v1/bcy/test/txs/push', JSON.stringify({tx: data})).then(function (response) {
