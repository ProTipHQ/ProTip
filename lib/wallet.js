    ...
    function updateBalance() {
        ...
        if (address.length) {
            ...
            preferences.getLastBalance().then(function (result) {
                ...
                util.get('https://blockchain.info/q/addressbalance/' + address + '?confirmations=1').then(function (response) {
                    balance = response;
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (decryptedPrivateKey) {
                ...
                util.getJSON('https://blockchain.info/unspent?address=' + address).then(function (json) {
                    ...
                    var outputsInputsObj = ret.ensureOutputsDoNotExceedInputs(txOutputs,inputs);
                    txOutputs = outputsInputsObj.txOutputs;
                    var selectedOuts = outputsInputsObj.selectedOuts;
                    var totalOutputsSatoshi = outputsInputsObj.totalOutputsSatoshi;
                    var totalInputsSatoshi = outputsInputsObj.totalInputsSatoshi;
                    var changeValue = totalInputsSatoshi.subtract(totalOutputsSatoshi);
