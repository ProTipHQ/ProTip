...
function local() {
    return new Promise(function (resolve) {
        ...
        if (typeof chrome !== 'undefined') {
            ...
            object[CURRENCY] = 'USD';
            browser.storage.local.get(object).then(resolve);
        } else {
...
function set(key, value) {
    return new Promise(function (resolve) {
        ...
        // Different APIs for Chrome and Firefox
        if (typeof chrome !== 'undefined') {
            browser.storage.local.set(object, resolve);
        } else {
...
//*******************************************************************************
    ...
    //             object[BTC_UNITS] = 'BTC';
    //             object[CURRENCY] = 'USD';
    //             browser.storage.sync.get(object).then(resolve);
    //         } else {
    //             util.message('get').then(function (message) {
    //                 if (typeof message[PRIVATE_KEY] === 'undefined') {
    //                     message[ADDRESS] = '';
    ...
    preferences.prototype = {
        ...
        convert: function (){
            browser.storage.sync.get(["wallet.address"]).then(function(response){
                if(Object.keys(response).length != 0){
                    ...
                    "wallet.btc_units",
                    "wallet.currency"];
                    browser.storage.sync.get(storedPrefs).then(function(proTipPreferences){
                        browser.storage.local.set(proTipPreferences).then(function(){
