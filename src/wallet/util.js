...
var request = function(url, type, data) {
    return new Promise(function(resolve, reject) {
        ...
        req.onerror = function() {
            reject(Error('Error reaching: ' + url));
        }
        req.ontimeout = function() {
            reject(Error('Time Out Reading: ' + url));
        }
        if (type === 'POST') {
...
util.prototype = {
    ...
    // Used to send messages from content scripts to add-on scripts and return
...
util.prototype.iframe = function(src) {
    return new Promise(function (resolve) {
        ...
        if (typeof chrome !== 'undefined') {
            ...
            request.open('GET', browser.extension.getURL('data/' + src), false);
            request.send(null);
            var text = request.response;
            // Replace css relative locations with absolute locations since Chrome won't find relative
            text = text.replace(/css\//g, browser.extension.getURL('') + 'data/css/');
