...
var startTime = null;
var updateTimeOnPageInterval = 1000 * 10;
...
browser.alarms.onAlarm.addListener(function() {
    if (localStorage['automaticDonate'] == "true") {
        ...
        Promise.all([
            ...
            paymentManager.payAll(localStorage['incidentalTotalFiat'], localStorage['subscriptionTotalFiat']).then(function() {
...
window.addEventListener("storage", function() {
    // Let the user see their available balance in the browserAction doesn't
    // have to be super accurate. Does not need to hit an API call constantly.
...
function checkIdleTime(newState) {
    ...
    if ((newState == "idle" || newState == "locked") && localStorage["paused"] == "false") {
...
function updateTime(url, seconds) {
    ...
    db.get('sites', url).done(function(record) {
        if (record && record.timeOnPage) {
            db.from('sites', '=', url).patch({
                timeOnPage: (parseInt(record.timeOnPage) + parseInt(seconds))
            });
        } else if(record) {
            db.from('sites', '=', url).patch({
                timeOnPage: parseInt(seconds)
            });
        }
    });
}
    ...
    if (request.action && request.action == "isBlacklisted") {
            ...
            browser.tabs.query({ active: true }).then(function(tabs) {
                ...
                if (blacklistFound) {
                    ...
                    hasKnownBtcAddress(request.url, function(record) {
                        // Give content script permission to scan DOM for BTC addresses
                        browser.tabs.sendMessage(tab.id, {
                            method: 'isBlacklisted',
                            response: false,
                            knownBTCAddress: record.bitcoinAddress
                        });
                    })
                }
            });
        ...
        browser.tabs.query({active: true}).then(function(tabs) {
            ...
            browser.browserAction.setIcon({
                path: 'assets/images/icon_48.png',
                tabId: tab.id
            });
        });
        ...
        db.get('sites', request.url).done(function(record) {
            if (!record) {
                ...
                db.from('sites', '=', record.url).patch({
                    bitcoinAddress: request.bitcoinAddress
                });
            }
        ...
        browser.tabs.query({active: true}).then(function(tabs) {
            ...
            if (request.source && request.source == 'metatag') {
                ...
                browser.browserAction.setBadgeText({
                    text: request.bitcoinAddress.trim().substring(0,4),
                    tabId: tab.id
                });
            }
            browser.browserAction.setIcon({
                path: 'assets/images/heart48x48.png',
                tabId: tab.id
            });
        });
    }
